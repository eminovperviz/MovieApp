...
using MovieApp.SharedKernel.Constants;
...
public class JwtService : IJwtService
{
    ...
    public ParseTokenResult ValidateToken(string token)
    {
        try
        {
            var jwtTokenHandler = new JwtSecurityTokenHandler();
            ...
            if (validatedToken is JwtSecurityToken jwtSecurityToken)
            {
                ...
                if (!result)
                {
                    throw new ApiException(HttpResponseStatusType.InvalidAccessToken);
                }
            }
            else
                throw new ApiException(HttpResponseStatusType.InvalidAccessToken);

            var nameIdentifier = principal.Claims.FirstOrDefault(x => x.Type == CustomClaimTypes.UserId);
            ...
            return new ParseTokenResult { IsValid = true, UserId = userId, ExpDate = expDate };
        }
        catch (Exception)
        {
            return new ParseTokenResult { IsValid = false };
        }
    }
